...
- **Verifying JS syntax after an edit**: a bare open/close-paren or -brace count on the whole template file is *not* a reliable syntax check — Hebrew UI copy legitimately contains stray `(`/`)` (e.g. "בינה מלאכותית (AI)"), so a mismatch there is frequently a false alarm, not a real error. To actually verify, extract just the `<script type="text/x-dc" ...>...</script>` block's JS (find its line range with `grep -n '<script\|</script>'`, `sed` that range out) and run it through `/System/Library/Frameworks/JavaScriptCore.framework/Versions/A/Helpers/jsc -e "$(cat extracted.js)"` (macOS ships this, no install needed) — a `ReferenceError: Can't find variable: DCLogic` means it parsed fine and only failed at runtime for lacking its base class in isolation, which is expected and fine; an actual `SyntaxError` means a real problem worth fixing before shipping.
- **Browser-automation gotchas discovered testing this app** (not a bug in the app itself, but will cost you time if you don't know it): (1) the `computer` tool's `left_click_drag`/generic mouse-drag does **not** dispatch real HTML5 `dragstart`/`dragover`/`drop` events — to test drag-and-drop (e.g. the לוז grid), dispatch synthetic `DragEvent`s with a shared `DataTransfer` via `javascript_tool` instead, or trust that a real user's physical drag will work once you've confirmed the event handlers are wired correctly. (2) The `confirmDelete` double-click-to-confirm pattern needs both clicks within ~3.5s of *real* wall-clock time — if you insert a `read_page`/screenshot/reasoning gap between them the arm times out and the second click just re-arms; issue both clicks in one tool-call batch with no gap to test it. (3) The `computer scroll` action on this app's custom horizontally-scrolling containers has, at least once, misfired as a click plus stray keystrokes into whatever was focused — if you see garbled text appear in a field you never typed into, suspect the test tooling before the app; re-verify with deliberate single clicks + screenshots rather than scroll gestures on these containers.
